...
public abstract class ModAbstractTreeFeature<TFC extends ModTreeConfig> extends Feature<TFC> {
    protected static FastNoise fastNoise;
    protected long seed;

    public static final Map<Block, Block> SPREADABLE_TO_NON_SPREADABLE = new HashMap<>();

    public ModAbstractTreeFeature(Codec<TFC> configCodec) {
        super(configCodec);
    }

    public static boolean canLogPlaceHere(IWorldGenerationBaseReader worldReader, BlockPos blockPos) {
        return worldReader.hasBlockState(blockPos, (state) -> state.getMaterial() == Material.AIR || state.getMaterial() == Material.WATER) ||
                FeatureUtil.isPlant(worldReader, blockPos);
    }
    ...
    public boolean canLogPlaceHereNether(IWorldGenerationBaseReader worldReader, BlockPos blockPos) {
        return worldReader.hasBlockState(blockPos, (state) -> state.getMaterial() == Material.AIR || state.getMaterial() == Material.WATER ||
                state.getMaterial() == Material.LAVA) || FeatureUtil.isPlant(worldReader, blockPos);
    }
    ...
    public void placeTrunk(BlockPos startPos, ModTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
    ...
    public void placeBranch(BlockPos startPos, ModTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (canLogPlaceHere(reader, pos)) {
            this.setFinalBlockState(startPos, config, blockSet, reader, pos, config.getTrunkProvider().getBlockState(random, pos), boundingBox);
        }
    ...
    public void placeLeaves(BlockPos startPos, ModTreeConfig config, Random random, Set<BlockPos> blockSet, ISeedReader reader, BlockPos pos, MutableBoundingBox boundingBox) {
        pos = getTransformedPos(config, startPos, pos);
        if (isAir(reader, pos)) {
    ...
    public void placeLeaves(BlockPos startPos, ModTreeConfig config, Random random, ISeedReader reader, int x, int y, int z, MutableBoundingBox boundingBox, Set<BlockPos> blockPos) {
    ...
    public static BlockPos getTransformedPos(ModTreeConfig config, BlockPos startPos, BlockPos pos) {
    ...
    public boolean canSaplingGrowHere(IWorldGenerationBaseReader reader, BlockPos pos) {
        return reader.hasBlockState(pos, (state) -> {
            Block block = state.getBlock();
            return block.isIn(BlockTags.LOGS) || block.isIn(BlockTags.LEAVES) || state.isAir() || state.getMaterial() == Material.PLANTS ||
                    state.getMaterial() == Material.TALL_PLANTS || state.getMaterial() == Material.OCEAN_PLANT ||
                    state.getMaterial() == Material.LEAVES || state.getMaterial() == Material.EARTH;
        });
    }
     ...
     */
    public static boolean isDesiredGroundDirtTag(IWorldGenerationBaseReader reader, BlockPos pos, ModTreeConfig config) {
    ...
    public static boolean isDesiredGroundSandTag(IWorldGenerationBaseReader reader, BlockPos pos, ModTreeConfig config) {
        if (config.isPlacementForced())
            return true;
    ...
    public void buildTrunk(ISeedReader reader, ModTreeConfig config, Random random, BlockPos operatingPos, int downRange) {
    ...
    public void setDisk(ISeedReader world, Random random, BlockPos pos, ModTreeConfig config) {
    ...
    public final void setFinalBlockState(BlockPos startPos, ModTreeConfig config, Set<BlockPos> changedBlocks, IWorldWriter worldIn, BlockPos pos, BlockState blockState, MutableBoundingBox boundingBox) {
    ...
    public void setBlockStateWithoutUpdates(BlockPos startPos, ModTreeConfig config, IWorldWriter worldWriter, BlockPos blockPos, BlockState blockState) {
        worldWriter.setBlockState(blockPos, blockState, 18);
    }

    public void setBlockStateWithoutUpdates(BlockPos startPos, ModTreeConfig config, IWorldWriter worldWriter, BlockPos blockPos, BlockState blockState, int flags) {
